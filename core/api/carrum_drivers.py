...
def _fetch_portal_driver_detail_http(name: str):
	...
	try:
		...
		logger.exception("portal driver detail request failed for lead=%s: %s", name, e)
	...
	if _is_account_not_found_for_lead_display_id(response, resp_body):
		logger.info("portal driver detail: no Carrum account for lead display ID %s", name)
...
def _validate_send_agreement_lead_fields(lead) -> None:
	...
	if missing:
		frappe.throw(_("Complete required fields before sending agreement: {0}").format(", ".join(missing)))
...
def _raise_update_driver_validation_error(exc: ValidationError) -> None:
	...
	summary = (
		v_errors[0]["message"] if len(v_errors) == 1 else _("{0} validation issues").format(len(v_errors))
	)
...
def view_agreement(digio_id: str):
	...
	if not response.ok:
		...
		frappe.throw(_("Carrum agreement PDF error ({0})").format(response.status_code))
	...
	if "pdf" not in content_type and "octet-stream" not in content_type:
		...
		if sample.lstrip().startswith("<"):
			logger.warning("view_agreement: non-PDF body for %s: %s", did, sample[:80])
...
def _scheme_qualifies_for_assignments(scheme_name: str) -> bool:
	"""Match frontend Assignments tab: vendor or double driver schemes."""
	s = (scheme_name or "").strip().lower()
	if not s:
		return False
	return "vendor" in s or "double driver" in s


@frappe.whitelist(allow_guest=True)
def scheme_change_webhook():
	"""
	Carrum webhook when a driver's scheme changes.

	- When ``currentSchemeName`` is *vendor* or *double driver*, clears
	  ``primary_lead`` on secondary leads linked to the CRM Lead for this
	  ``accountId``.
	- Updates the lead's scheme fields from the webhook payload.
	"""
	payload = frappe.request.get_json() or {}
	logger.info("scheme_change_webhook payload: %s", payload)
	account_id = (payload.get("accountId") or "").strip()
	if not account_id:
		return {"message": "Ok", "skipped": "no accountId"}

	lead_id = frappe.db.get_value("CRM Lead", {"custom_account_id": account_id}, "name")

	if not lead_id:
		logger.info("scheme_change_webhook: no CRM Lead for accountId=%s", account_id)
		return {"message": "Ok", "skipped": "lead not found"}
	...
	if _scheme_qualifies_for_assignments(old_scheme_name):
		logger.info(
			"scheme_change_webhook: leaving assignment scheme %r, unassigning secondaries for lead=%s",
			old_scheme_name,
			lead_id,
		)
		result = util_service.un_assign_secondary_lead_from_lead(lead_id)
		unassigned = bool(result and result.get("cleared", 0) > 0)

	updates = {}
	if new_scheme_name:
		updates["scheme_name"] = new_scheme_name
	if new_scheme_id:
		updates["scheme_id"] = new_scheme_id
	if new_car_type_id:
		updates["car_type_id"] = new_car_type_id
	...
	logger.info(
		"scheme_change_webhook: lead=%s account=%s old=%s new=%s unassigned=%s payload_keys=%s",
		lead_id,
		account_id,
		old_scheme_name,
		new_scheme_name,
	...
	return {
		"message": "Ok",
		"lead": lead_id,
		"unassigned_secondaries": unassigned,
	}
